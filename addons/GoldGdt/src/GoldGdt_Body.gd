...
func _physics_process(delta) -> void:
	...
	if is_on_floor():
		...
		dash_direction = get_real_velocity()
		if Input.is_action_just_pressed("pm_dash") && velocity != Vector3.ZERO && can_dash == true:
			can_dash = false
			
			velocity.x += dash_direction.x * 2
			velocity.z += dash_direction.z * 2
			
			var cam_tween = get_tree().create_tween()
			var vm_tween = get_tree().create_tween()
			cam_tween.tween_property($"../Interpolated Camera/Arm/Arm Anchor/Camera Recoil/Camera Shake/Camera", "position:y", -0.3, 0.3)
			cam_tween.tween_property($"../Interpolated Camera/Arm/Arm Anchor/Camera Recoil/Camera Shake/Camera", "position:y", 0, 0.3)
			vm_tween.tween_property($"../Interpolated Camera/Arm/Arm Anchor/viewmodels", "position:y", -0.34, 0.3)
			vm_tween.tween_property($"../Interpolated Camera/Arm/Arm Anchor/viewmodels", "position:y", -0, 0.3)
